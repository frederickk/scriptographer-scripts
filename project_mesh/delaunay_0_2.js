...
var components = {
	...
	triangulate: { 
		...
		onClick: function() {
			...
			triangulateSelected(sel);
			if(points.length >= 3) Draw();
		}
	},
...
function Setup() {
	RasterSetup();
	var palette = new Palette('Delaunay 0.2', components, values);
};
...
function Draw() {
	...
	triangles = new Triangulate( points );
};
...
// ------------------------------------------------------------------------
function addDelaunayPoint( pt ) {
	points.push( pt );
	Draw();
};

// ------------------------------------------------------------------------
function triangulateSelected(items) {
	console.log( 'triangulateSelected()' );
	var temp = [];
...
function createFaces(triangles) {
	...
	for( i in triangles ) {
		...
		// var density = Math.round(values.densityMax * averageBrightness(colPt,2).gray );
		...
		//	line.fillColor = null;
		//	line.opacity = values.opacity/100;
